...
class Clients extends CI_Controller {
	...
	public function edit($id)
	{
		...
		$this->form_validation->set_rules('kontaktart',LTEXT('_kontaktarts'),'required');
		$this->form_validation->set_rules('status',LTEXT('_status'),'');
		$this->form_validation->set_rules('bewertung',LTEXT('_bewertung'),'');
		$this->form_validation->set_rules('kunde',LTEXT('_kunde'),'');
		$this->form_validation->set_rules('KDnr',LTEXT('_KDnr'),'');
		$this->form_validation->set_rules('eigentuemer',LTEXT('_eigentuemer'),'');
		$this->form_validation->set_rules('lister',LTEXT('_lister'),'');
		$this->form_validation->set_rules('titel',LTEXT('_titel'),'');
		$this->form_validation->set_rules('vorname',LTEXT('_vorname'),'required|callback_address_name_check');
		$this->form_validation->set_rules('name',LTEXT('_name'),'required');
		$this->form_validation->set_rules('firma',LTEXT('_firma'),'');
		$this->form_validation->set_rules('position',LTEXT('position'),'');
		$this->form_validation->set_rules('adresse_1',LTEXT('_adresse_1'),'');
		$this->form_validation->set_rules('block',LTEXT('_block'),'');
		$this->form_validation->set_rules('etage',LTEXT('_etage'),'');
		$this->form_validation->set_rules('tuer',LTEXT('_tuer'),'');
		$this->form_validation->set_rules('plz',LTEXT('_plz'),'');
		$this->form_validation->set_rules('ort',LTEXT('_ort'),'');
		$this->form_validation->set_rules('land',LTEXT('_land'),'');
		$this->form_validation->set_rules('anrede',LTEXT('_anrede'),'');
		$this->form_validation->set_rules('briefanrede',LTEXT('_briefanrede'),'');
		$this->form_validation->set_rules('newsletter',LTEXT('newsletter'),'');
		$this->form_validation->set_rules('sprache',LTEXT('_sprache'),'');
		$this->form_validation->set_rules('resident',LTEXT('_resident'),'');
		$this->form_validation->set_rules('kundenaquise',LTEXT('_kundenaquise'),'');
		$this->form_validation->set_rules('notiz',LTEXT('_notiz'),'');
		
		if($this->form_validation->run() == true)
		{
			$update_data['modify_datum'] = date('Y-m-d H:i:s');
			$update_data['aenderungsperson'] = $this->session->userdata['userid'];
			$update_data['kontaktart'] = $this->input->post('kontaktart');
			$update_data['status'] = $this->input->post('status');
			$update_data['bewertung'] = $this->input->post('bewertung');
			$update_data['kunde'] = $this->input->post('kunde');
			$update_data['KDnr'] = $this->input->post('KDnr');
			$update_data['eigentuemer'] = $this->input->post('eigentuemer');
			$update_data['lister'] = $this->input->post('lister');
			$update_data['titel'] = $this->input->post('titel');
			$update_data['vorname'] = $this->input->post('vorname');
			$update_data['name'] = $this->input->post('name');
			$update_data['firma'] = $this->input->post('firma');
			$update_data['position'] = $this->input->post('position');
			$update_data['adresse_1'] = $this->input->post('adresse_1');
			$update_data['block'] = $this->input->post('block');
			$update_data['etage'] = $this->input->post('etage');
			$update_data['tuer'] = $this->input->post('tuer');
			$update_data['plz'] = $this->input->post('plz');
			$update_data['ort'] = $this->input->post('ort');
			$update_data['land'] = $this->input->post('land');
			$update_data['anrede'] = $this->input->post('anrede');
			$update_data['briefanrede'] = $this->input->post('briefanrede');
			$update_data['sprache'] = $this->input->post('sprache');
			$update_data['newsletter'] = $this->input->post('newsletter');
			$update_data['resident'] = $this->input->post('resident');
			$update_data['kundenaquise'] = $this->input->post('kundenaquise');
			$update_data['notiz'] = $this->input->post('notiz');
			$this->clients_model->update_addresses($update_data,$id);
			$this->session->set_flashdata('success_msg',LTEXT('_address_updated_successfuly'));
		...
		$data['addressess_where_kontaktart_6'] = $this->clients_model->get_addressess_where_kontaktart_6();
	...
	public function load_profiles_ajax($id,$id_address)
	{
		...
		if($SearchCategory){
			...
			if($SC2)$ot2 = " OR ob_kategorien_1 = $SC2 OR ob_kategorien_2 = $SC2 OR ob_kategorien_3 = $SC2";
			$KatCond="AND ( $ot1 $ot2 )";
		}
		$s = $profile->region;
		if($s){
		 ...
		 * 
		 */
		if( !isset($KatCond) ) $KatCond = '';
		if( !isset($RegionCond) ) $RegionCond = '';
		if( !isset($ATTRIB_CONTENT_WHERE) ) $ATTRIB_CONTENT_WHERE = '';
		if( !isset($PERM_COND) ) $PERM_COND = '';
		if( !isset($SACHBEARBEITER_JOIN) ) $SACHBEARBEITER_JOIN = '';
